...
export const getAcceptedBlogs = async (req: Request, res: Response) => {
  ...
  if (!blogId || !email) return res.status(400).json({ error: "Missing parameters." });

  try {
    const blog = await prisma.blog.findUnique({ where: { id: blogId } });
    if (!blog) return res.status(404).json({ error: "Blog not found." });
    if (blog.writerEmail !== email) return res.status(403).json({ error: "Unauthorized." });
    if (!blog.isAccepted) return res.status(400).json({ message: "Not accepted yet." });
